...
def iter_next_image(loc_image, loc_caption, list_image_path):
    ...
    with open(curr_caption_path, 'r') as f:
        caption_prev = f.read()
    loc_caption.text_area("Caption:", value=caption_prev)
...
caption_input = loc_caption.text_area("Caption:", value=caption_prev, height=200)

# initialize buttons
col1, col2 = st.columns([1, 1])
with col1:
    if st.button('Save', type='primary'):
        fn_image = os.path.basename(curr_image_path)
        fn_caption = os.path.basename(curr_caption_path)

        output_image_path = os.path.join(args.output_dir, fn_image)
        output_caption_path = os.path.join(args.output_dir, fn_caption)

        shutil.copyfile(curr_image_path, output_image_path)
        shutil.copyfile(curr_caption_path, output_caption_path)
